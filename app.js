  ...
  return (
    <input
      ...
      style={{
        ...
        borderRadius:"8px", padding:"8px 12px", color:"#e5e7eb", fontSize:"14px",
  ...
  return (
    <textarea
      ...
      style={{
        ...
        borderRadius:"10px", padding:"10px 12px", color:"#e5e7eb", fontSize:"14px",
...
function ActivityCard({ act, weekend, isEditing, onEdit, onChange, onRemove }) {
  const fields = [
  ...
  return (
    <div style={{ background:"rgba(255,255,255,0.04)", border:`1px solid ${weekend.color}30`, borderRadius:"12px", padding:"14px", marginBottom:"10px" }}>
      {!isEditing ? (
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"8px" }}>
          ...
          <div style={{ display:"flex", gap:"6px", flexShrink:0 }}>
            <button onClick={onEdit} style={{ background:`${weekend.color}22`, border:"none", borderRadius:"8px", padding:"7px 12px", color:weekend.color, cursor:"pointer", fontSize:"13px", fontWeight:"600" }}>Edit</button>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span style={{ fontSize:"11px", color:weekend.color, fontWeight:"700", letterSpacing:"1px", textTransform:"uppercase" }}>Editing Activity</span>
  ...
  WEEKENDS.forEach(we => {
    we.days.forEach(day => {
      (data[day]?.activities || []).forEach(act => {
        ...
        if (c > 0) { allActs.push({ ...act, day, weekend: we, c }); grandTotal += c; }
      });
    });
  });
  ...
  return (
    <div>
        ...
        <div style={{ marginBottom:"16px" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
            ...
            <input
              type="number"
              value={budget.total}
              onChange={e => onBudgetChange({ total: e.target.value })}
              placeholder="0.00"
      ...
      <div style={{ marginBottom:"20px" }}>
        ...
        {WEEKENDS.map(we => {
          const weActs = allActs.filter(a => a.weekend.id === we.id);
            ...
            <div key={we.id} style={{ background:`${we.color}10`, border:`1px solid ${we.color}30`, borderRadius:"14px", padding:"16px", marginBottom:"12px" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"10px" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"4px" }}>
                  ...
                  <input
                    type="number"
                    value={budget[`we${we.id}`]||""}
                    onChange={e => onBudgetChange({ [`we${we.id}`]: e.target.value })}
                    placeholder="Budget"
...
function App() {
  ...
  const [activeWeekend, setActiveWeekend] = useState(0);
  ...
  const we = WEEKENDS[activeWeekend];
  ...
  const handleCalClick = day => {
    const w = getWeekendForDay(day);
    if (w) {
      setActiveWeekend(WEEKENDS.indexOf(w));
      setSelectedDay(day);
      setMainTab(0);
        ...
        {mainTab === 0 && (
          <div>
                ...
                <div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"4px" }}>
                    {cells.map((day,i) => {
                      ...
                      const w=getWeekendForDay(day), sel=selectedDay===day, bd=isBirthday(day), hasA=getDayData(day).activities.length>0;
                      return (
                        <div key={day} onClick={() => handleCalClick(day)}
                          style={{ aspectRatio:"1", borderRadius:"8px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", cursor:w?"pointer":"default", position:"relative", background:bd?"linear-gradient(135deg,#ffd700,#ff8c00)":w?(sel?w.color:`${w.color}28`):"rgba(255,255,255,0.03)", border:bd?"2px solid #ffd700":sel?`2px solid ${w?.color||"#fff"}`:"2px solid transparent", transform:sel?"scale(1.1)":"scale(1)", transition:"all 0.18s", boxShadow:bd?"0 0 18px rgba(255,215,0,0.4)":sel?`0 0 12px ${w?.color}55`:"none" }}>
                          <span style={{ fontSize:"clamp(11px,2vw,14px)", fontWeight:bd?"900":w?"700":"400", color:bd?"#2a1000":w?(sel?w.textColor:w.color):"#4b5563", lineHeight:1 }}>{bd?"👑":day}</span>
                          {bd && <span style={{ fontSize:"9px", color:"#2a1000", fontWeight:"800" }}>24</span>}
                          {hasA && <div style={{ width:"5px", height:"5px", borderRadius:"50%", background:w?.color||"#fff", position:"absolute", bottom:"3px" }} />}
                        </div>
                      );
                    })}
                  </div>
            ...
            <div style={{ display:"flex", gap:"6px", overflowX:"auto", paddingBottom:"6px", marginBottom:"20px" }}>
              {WEEKENDS.map((w,i) => (
                <button key={w.id} onClick={() => { setActiveWeekend(i); setSelectedDay(null); }}
                  style={{ padding:"10px 18px", borderRadius:"30px", border:"none", cursor:"pointer", background:activeWeekend===i?w.color:"rgba(255,255,255,0.07)", color:activeWeekend===i?w.textColor:"#9ca3af", fontWeight:"700", fontSize:"13px", whiteSpace:"nowrap", fontFamily:"inherit", boxShadow:activeWeekend===i?`0 4px 24px ${w.color}55`:"none", transition:"all 0.2s" }}>
                  {w.label}
                </button>
              ))}
            </div>

            <div style={{ background:`linear-gradient(135deg,${we.color}1a,${we.accent}0d)`, border:`1px solid ${we.color}40`, borderRadius:"14px", padding:"14px 18px", marginBottom:"20px", display:"flex", alignItems:"center", gap:"12px" }}>
              <div style={{ width:"14px", height:"14px", borderRadius:"4px", background:we.color, flexShrink:0 }} />
              <div>
                <div style={{ fontWeight:"700", fontSize:"17px", color:we.color }}>{we.name}</div>
                <div style={{ fontSize:"12px", color:"#9ca3af" }}>May {we.days[0]}–{we.days[we.days.length-1]}, 2026</div>
              </div>
            </div>

            {we.days.map(day => {
              const dow=getDayOfWeek(day), dayData=getDayData(day), bd=isBirthday(day);
              return (
                <div key={day} id={`dc-${day}`} style={{ background:bd?"linear-gradient(135deg,#2d0a00,#0f0500)":"rgba(255,255,255,0.04)", border:bd?"2px solid #ffd700":`1px solid ${we.color}28`, borderRadius:"18px", padding:"20px", marginBottom:"18px", boxShadow:bd?"0 0 40px rgba(255,215,0,0.1)":"none" }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"16px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                      <div style={{ width:"54px", height:"54px", borderRadius:"13px", flexShrink:0, background:bd?"linear-gradient(135deg,#ffd700,#ff8c00)":we.color, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", boxShadow:bd?"0 0 24px rgba(255,215,0,0.5)":`0 4px 16px ${we.color}44` }}>
                        <span style={{ fontSize:bd?"22px":"20px", fontWeight:"800", color:bd?"#2a1000":we.textColor, lineHeight:1 }}>{bd?"👑":day}</span>
                        <span style={{ fontSize:"10px", color:bd?"#5a3000":we.textColor, opacity:0.85, fontWeight:"600" }}>{bd?"24":DAY_NAMES[dow]}</span>
                      </div>
                      <div>
                        <div style={{ fontWeight:"700", fontSize:"17px", color:"#f9fafb" }}>{bd?"Your Birthday 👑":`${DAY_NAMES[dow]}, May ${day}`}</div>
                        <div style={{ fontSize:"13px", color:bd?"#ffd700":"#9ca3af" }}>{bd?"No commitments. This day is YOURS.":`${dayData.activities.length} activit${dayData.activities.length===1?"y":"ies"}`}</div>
                      </div>
                    </div>
                    {!bd && (
                      <button onClick={() => addAct(day)}
                        style={{ background:we.color, color:we.textColor, border:"none", borderRadius:"20px", padding:"9px 18px", fontSize:"13px", fontWeight:"700", cursor:"pointer", fontFamily:"inherit", flexShrink:0 }}>
                        + Add
                      </button>
                    )}
                  </div>
                  {dayData.activities.length === 0 && !bd && (
                    <div style={{ textAlign:"center", padding:"20px", color:"#4b5563", fontSize:"14px", fontStyle:"italic" }}>Nothing planned yet. The canvas is blank ✨</div>
                  )}
                  {dayData.activities.map(act => (
                    <ActivityCard key={act.id} act={act} weekend={we}
